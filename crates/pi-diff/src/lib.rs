//! jsdiff-compatible diff primitives (pure Rust core).
//!
//! # Overview
...
//!
//! Everything operates on UTF-16 code units end to end — `&[u16]` /
//! [`Vec<u16>`] here, which is the exact value space of JS strings; the N-API
//! shell in `pi-natives` bridges [`napi::bindgen_prelude::Utf16String`] on top.
//! Ill-formed input (unpaired surrogates) is legal content that diffs
//! code-unit-for-code-unit like jsdiff, so callers never need a JS fallback,
//! and no UTF-8 conversion happens in either direction.
...
/// from the new text.
#[must_use]
pub fn diff_lines(old_text: &[u16], new_text: &[u16]) -> Vec<DiffChange> {
...
#[must_use]
pub fn diff_line_runs(old_text: &[u16], new_text: &[u16]) -> Vec<DiffRun> {
...
#[must_use]
pub fn diff_words(old_text: &[u16], new_text: &[u16]) -> Vec<DiffChange> {
...
mod tests {
	...
	fn structured_patch_marks_missing_eof_newline() {
		let hunks = structured_patch_hunks(&u16s("a\nb"), &u16s("a\nc"), Some(3));
	...
	fn word_diff_dedupes_boundary_whitespace() {
		...
		let changes = diff_words(&u16s("foo bar baz"), &u16s("foo qux baz"));
	...
	fn line_runs_preserve_empty_lines() {
		let runs = diff_line_runs(&u16s("a\n\nb"), &u16s("a\n\nc"));
	...
	fn unpaired_surrogates_diff_as_distinct_content() {
		...
		let shaped: Vec<(Vec<u16>, bool, bool)> = diff_lines(&old, &new)
			.into_iter()
			.map(|c| (c.value.clone(), c.added, c.removed))
			.collect();
	...
	fn word_scan_keeps_lone_surrogate_before_astral_pair_separate() {
		...
		let shaped: Vec<(Vec<u16>, bool, bool)> = diff_words(&old, &new)
			.into_iter()
			.map(|c| (c.value.clone(), c.added, c.removed))
			.collect();
		assert_eq!(shaped, vec![
