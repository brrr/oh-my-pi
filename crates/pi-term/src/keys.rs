//! Kitty keyboard sequence matching utilities (pure engine).
//!
//! # Overview
//! Parses Kitty keyboard protocol sequences and matches codepoints plus
//! modifiers. This is the pure Rust core; the napi shell over it lives in
//! `pi-natives::keys`.
//!
//! # Example
//! ```ignore
//! // matches_kitty_sequence(b"\x1b[65;5u", 65, 4) -> true
//! // parse_key_inner(b"\x1b[65;5u", false) -> Some("ctrl+a")
//! ```
...
pub struct ParsedKittySequence {
	...
	/// Optional text-as-codepoints field (single code point when present).
	pub text_codepoint:  Option<i32>,
	/// Modifier bitmask (shift/alt/ctrl/super plus lock bits).
	pub modifier:        u32,
...
pub fn matches_key_inner(bytes: &[u8], key_id: &str, kitty_protocol_active: bool) -> bool {
...
#[inline]
pub fn parse_key_inner(bytes: &[u8], kitty_protocol_active: bool) -> Option<Cow<'static, str>> {
...
pub fn parse_kitty_sequence_bytes(bytes: &[u8]) -> Option<ParsedKittySequence> {
