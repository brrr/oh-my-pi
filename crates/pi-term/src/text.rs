//! ANSI-aware text measurement and slicing utilities (pure engine).
//!
//! Optimized for JS string interop (UTF-16). The data plane is `&[u16]` /
//! `Vec<u16>`, the exact value space of JS strings; the napi shell over this
//! (`JsString` boundary, zero-copy fast paths, `Utf16String` DTOs) lives in
//! `pi-natives::text`.
...
//! - [`truncate_to_width`] returns `None` when the input is unchanged, so the
//!   napi shell can hand back the original `JsString` with zero allocation.
...
#[inline]
pub fn clamp_tab_width_for_ops(width: u32) -> usize {
...
pub fn visible_width_u16(data: &[u16], tab_width: usize) -> usize {
...
pub fn wrap_text_with_ansi_impl(
	text: &[u16],
	width: usize,
	tab_width: usize,
) -> SmallVec<[Vec<u16>; 4]> {
...
/// Truncate `text` to a visible `max_width`, preserving ANSI codes and padding
/// with spaces when `pad` is set.
///
/// Returns `None` when the input needs no change (fits within `max_width` and
/// either no padding is requested or it already fills the width), so the napi
/// shell can return the original `JsString` handle with zero allocation.
/// Returns `Some(out)` (UTF-16, unstripped) otherwise.
pub fn truncate_to_width(
	...
	if !exceeded {
		if !pad {
			// Unchanged: caller returns the original JsString, zero allocation.
			return None;
		}
		...
		// Exactly fits and padding requested: returning original is still fine.
		return None;
	}
