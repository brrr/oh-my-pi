//! Snapcompact frame rendering (pure engine).
//!
...
//! is only the hot `text -> PNG bytes` path. The napi shell over it (base64 +
//! `Latin1String` boundary) lives in `pi-natives::snapcompact`.
...
/// Shape options for one snapcompact frame. Pure-Rust mirror of the napi
/// `SnapcompactRenderOptions` DTO in `pi-natives::snapcompact`.
#[derive(Default)]
...
pub fn supported_chars(font: &str, chars: &str) -> Result<String> {
	let resolved = resolve_font(font).ok_or_else(|| {
		anyhow!(
	...
	for ch in chars.chars() {
		if matches!(ch as u32, DIM_ON | DIM_OFF | FULL_BLOCK | 0x0a) || resolved.supports(ch as u32) {
			supported.push(ch);
		}
	}
	Ok(supported)
...
/// Render one snapcompact frame: print pre-normalized text onto a `size`-wide
/// bitmap and encode it as PNG, returning the raw PNG bytes.
///
...
/// cell.
pub fn render_snapcompact_png_sync(text: &str, options: RenderOptions) -> Result<Vec<u8>> {
	...
	let black_ink = match options.variant.as_deref().unwrap_or("sent") {
		...
		"bw" => true,
		other => {
			return Err(anyhow!("Unknown snapcompact variant {other:?}: expected \"sent\" or \"bw\""));
		},
	};
...
mod tests {
	...
	/// Identity helper: the engine returns raw PNG bytes (base64 +
	/// `Latin1String` live in the napi shell), so tests inspect the bytes
	/// directly.
	fn png_bytes(png: Vec<u8>) -> Vec<u8> {
		png
	}
	...
	fn render_native_is_indexed_and_stretch_is_rgb() {
		let native = png_bytes(
			render_snapcompact_png_sync("Hello world. Again.", RenderOptions {
				size: 128,
				font: Some("8x8".into()),
		...
		let legacy = png_bytes(render_snapcompact_png_sync("Hi. Ok.", opts(40)).unwrap());
		assert_eq!(legacy[25], 3, "default shape stays the legacy 5x8 indexed path");

		let silver = png_bytes(
			render_snapcompact_png_sync("こんにちは 你好 안녕", RenderOptions {
				size: 128,
	...
	fn indexed_png_narrows_palette_and_bit_depth() {
		...
		let dim = png_bytes(
			render_snapcompact_png_sync("Read \u{e}the dim part\u{f} now.", RenderOptions {
				size: 128,
				font: Some("8x8".into()),
				variant: Some("bw".into()),
				line_repeat: Some(2),
		...
		let sent = png_bytes(
			render_snapcompact_png_sync("Hi. Ok.", RenderOptions {
				size: 128,
				font: Some("8x8".into()),
	...
	fn rejects_bad_shapes() {
		assert!(render_snapcompact_png_sync("x", opts(0)).is_err());
		assert!(
			render_snapcompact_png_sync("x", RenderOptions {
				size: 64,
				font: Some("9x9".into()),
				..Default::default()
			})
	...
	fn stretch_false_renders_natural_glyphs_on_padded_pitch() {
		let png = png_bytes(
			render_snapcompact_png_sync("Hello there. General Kenobi!", RenderOptions {
				size: 128,
				font: Some("8x13".into()),
				cell_width: Some(8),
				cell_height: Some(16),
	...
	fn frame_height_hugs_used_rows() {
		...
		let render = |text: &str, opts: RenderOptions| {
			png_bytes(render_snapcompact_png_sync(text, opts).unwrap())
		};
		let opts_8x8 = || RenderOptions { size: 64, font: Some("8x8".into()), ..Default::default() };
		...
		let repeated = render("0123456789", RenderOptions { line_repeat: Some(2), ..opts_8x8() });
	...
	fn columns_validates_and_renders_doc_frames() {
		...
		let doc = png_bytes(
			render_snapcompact_png_sync("Hello there.\nSecond line", RenderOptions {
				size: 256,
				font: Some("8x13".into()),
				cell_width: Some(8),
				cell_height: Some(16),
