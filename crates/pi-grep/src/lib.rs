//! Ripgrep-backed search engine (pure Rust core, no N-API).
//!
//! Provides two layers:
...
//!
//! `pi-natives` wraps these entry points in a thin N-API shell that maps the
//! pure types here to `#[napi(object)]` DTOs and errors to `napi::Error`; the
//! error message strings are preserved verbatim across that boundary.
...
/// Streaming per-match callback fired by [`grep`] in directory mode.
///
/// Mirrors the `ChunkFn` shape used by the group A pty extraction: a boxed,
/// `Send` closure so callers (the N-API shell) can wrap a `ThreadsafeFunction`
/// without leaking napi types into the engine.
pub type OnMatchFn = Box<dyn Fn(&GrepMatch) + Send>;
...
/// Converts a native walker error into an engine error, preserving the
/// TS-visible message text.
fn map_walker_error<E: std::fmt::Display>(err: pi_walker::WalkError<E>) -> anyhow::Error {
	match err {
		pi_walker::WalkError::Interrupted(err) => anyhow!("{err}"),
		pi_walker::WalkError::InvalidData { path, message } => {
			anyhow!("Native directory scan failed for {}: {message}", path.display())
		},
	}
}

/// Output mode for [`search`] and [`grep`].
...
/// Configuration for [`grep`], extracted from options.
...
const fn parse_output_mode(mode: Option<OutputMode>) -> OutputMode {
	match mode {
		None | Some(OutputMode::Content) => OutputMode::Content,
		Some(OutputMode::Count) => OutputMode::Count,
		Some(OutputMode::FilesWithMatches) => OutputMode::FilesWithMatches,
	}
}
...
fn build_matcher(pattern: &str, ignore_case: bool, multiline: bool) -> Result<CompiledMatcher> {
	...
	build_regex_matcher(&regex::escape(pattern), ignore_case, multiline)
		...
		.map_err(|_| anyhow!("Regex error: {message}"))
}
...
fn build_grep_walk_request(
	...
	if let Some(glob) = glob.map(str::trim).filter(|value| !value.is_empty()) {
		let pattern = glob::build_glob_pattern(glob, true);
...
fn collect_grep_candidates(
	...
	let mut candidates = match request.collect_file_candidates_with_heartbeat(|| ct.heartbeat()) {
		...
		Err(err) => return Err(map_walker_error(err)),
	};
...
fn push_count_match(matches: &mut Vec<GrepMatch>, path: String, match_count: u64) {
	matches.push(GrepMatch {
		...
		context_after: None,
		truncated: None,
		match_count: Some(clamp_u32(match_count)),
	});
}
...
fn aggregate_parallel_results(
	...
	let files_searched = clamp_u32(files_searched);
...
pub fn search(content: &[u8], options: SearchOptions) -> SearchResult {
...
///
/// When `on_match` is provided, each returned match (directory mode only, after
/// offset/limit aggregation) is streamed through the callback before the
/// aggregated result is returned.
pub fn grep(
	options: GrepConfig,
	on_match: Option<OnMatchFn>,
	ct: &CancelToken,
) -> Result<GrepResult> {
	let ignore_case = options.ignore_case.unwrap_or(false);
	let multiline = options.multiline.unwrap_or(false);
	match build_matcher(&options.pattern, ignore_case, multiline)? {
		CompiledMatcher::Rust(matcher) => grep_with_matcher(options, on_match, ct, &matcher),
		CompiledMatcher::Pcre(matcher) => grep_with_matcher(options, on_match, ct, &matcher),
	}
}

fn grep_with_matcher<M: Matcher + Sync>(
	options: GrepConfig,
	on_match: Option<OnMatchFn>,
	ct: &CancelToken,
	matcher: &M,
	...
	let metadata =
		std::fs::metadata(&search_path).map_err(|err| anyhow!("Path not found: {err}"))?;
	let multiline = options.multiline.unwrap_or(false);
	...
	let _ = glob::try_compile_glob(glob, true)?;
	...
	// results.
	if let Some(callback) = on_match.as_ref() {
	...
	Ok(GrepResult {
		...
		skipped_oversized: if skipped_oversized > 0 {
			Some(clamp_u32(skipped_oversized))
		} else {
			None
		},
	})
...
mod tests {
	...
	#[cfg(unix)]
	use super::{GrepConfig, OutputMode, grep};
	...
	fn grep_supports_pcre2_lookaround_and_backreferences() {
		...
		for (pattern, path, line) in [
			...
			let result = grep(config, None, &CancelToken::default())
	...
	fn grep_directory_skips_fifo_entries() {
		...
		let result = grep(base_grep_config(root.path()), None, &CancelToken::default())
	...
	fn grep_sync_with_gitignore_skips_ignored_rs_files() {
		...
		let result =
			grep(config, None, &CancelToken::default()).expect("gitignore-aware grep should succeed");
	...
	fn grep_files_with_matches_counts_all_searched_files_when_none_match() {
		...
		let mut config = base_grep_config(root.path());
		config.mode = Some(OutputMode::FilesWithMatches);

		let result =
			grep(config, None, &CancelToken::default()).expect("directory grep should succeed");
	...
	fn grep_count_mode_limit_applies_to_matches_not_files() {
		...
		let mut config = base_grep_config(root.path());
		config.mode = Some(OutputMode::Count);
		config.max_count = Some(2);
	...
	fn grep_streaming_respects_pre_cancelled_token() {
		...
		let result = grep(base_grep_config(root.path()), None, &ct);
	...
	fn grep_special_root_path_returns_empty_result() {
		...
		let result = grep(base_grep_config(&fifo), None, &CancelToken::default())
	...
	fn grep_multiline_matches_cross_line_patterns() {
		...
		let result =
			grep(config, None, &CancelToken::default()).expect("multiline grep should succeed");
	...
	fn sequential_reference_result(root: &Path, params: super::SearchParams) -> super::GrepResult {
		...
		super::GrepResult {
			matches,
			total_matches: super::clamp_u32(total_matches),
			files_with_matches,
			files_searched,
			limit_reached: limit_reached.then_some(true),
			skipped_oversized: (skipped_oversized > 0).then(|| super::clamp_u32(skipped_oversized)),
		}
	}
	...
	fn parallel_streaming_count_and_files_modes_match_sequential_reference() {
		...
		for (mode, output_mode) in [
			(super::OutputMode::Count, OutputMode::Count),
			(super::OutputMode::FilesWithMatches, OutputMode::FilesWithMatches),
		] {
			let mut config = base_grep_config(root.path());
			config.gitignore = Some(true);
			...
			let actual =
				grep(config, None, &CancelToken::default()).expect("parallel grep should succeed");
	...
	fn windowed_streaming_skips_oversized_when_normal_matches_satisfy_budget() {
		...
		let result =
			grep(config, None, &CancelToken::default()).expect("windowed grep should succeed");
		let paths: Vec<&str> = result
			.matches
			.iter()
			.map(|matched| matched.path.as_str())
	...
	fn single_oversized_target_searches_prefix() {
		...
		let result = grep(base_grep_config(&big), None, &CancelToken::default())
