//! File-discovery orchestration for ast-grep search and rewrite (pure Rust
//! core).
//!
//! The matching/edit engine proper lives in [`crate::ops`]; this module owns
//! the orchestration around it: directory walking + glob filtering, per-file
//! ast operations, bounded top-K result retention, and result aggregation. It
//! is N-API-free — the `pi-natives` shell wraps these entry points with the
//! `#[napi(object)]` DTOs and `ThreadsafeFunction`/cancel bridging.
//!
//! Cancellation flows through [`pi_shell::cancel::CancelToken`]; glob filtering
//! reuses [`pi_grep::glob`]; errors are [`anyhow`]. TS-visible error strings
//! (including [`map_walker_error`] output) are preserved verbatim so the napi
//! shell surfaces identical messages.
...
use crate::{
	SupportLang,
	ops::{
		apply_edits, compile_pattern, is_supported_file, resolve_language, resolve_supported_lang,
	},
};

const DEFAULT_FIND_LIMIT: u32 = 50;

/// Parameters for [`find`]: patterns, scan scope, and match limits.
pub struct AstFindParams {
	...
	/// Pattern strictness (already resolved from the caller's default).
...
/// Parameters for [`match_source`]: run ast-grep patterns against an in-memory
/// source string instead of files on disk.
pub struct AstMatchParams {
...
/// Result of an in-memory [`match_source`] run.
...
/// Parameters for [`edit`]: rewrite rules, scan scope, safety limits, and
/// dry-run.
pub struct AstReplaceParams {
...
/// Per-file replacement count after an [`edit`] run.
...
/// Convert a native walker error into an [`anyhow::Error`], preserving the
/// exact message strings the N-API shell historically surfaced (see the old
/// `iofs::map_walker_error`).
...
fn infer_single_replace_lang(candidates: &[FileCandidate], ct: &CancelToken) -> Result<String> {
	...
	if inferred.is_empty() {
		return Err(anyhow!("`lang` is required for ast_edit when no files match path/glob"));
	}
	if inferred.len() > 1 {
		return Err(anyhow!(
...
fn normalize_search_path(path: Option<String>) -> Result<PathBuf> {
	...
	let absolute = if candidate.is_absolute() {
		...
		std::env::current_dir()
			.map_err(|err| anyhow!("Failed to resolve cwd: {err}"))?
			.join(candidate)
	};
...
fn collect_candidates(
	...
	if !metadata.is_dir() {
		return Err(anyhow!("Search path must be a file or directory: {}", search_path.display()));
	}
...
fn normalize_rewrite_map(
	...
	for (pattern, rewrite) in rewrites.unwrap_or_default() {
		if pattern.is_empty() {
			return Err(anyhow!("`rewrites` keys must be non-empty pattern strings"));
		}
...
/// Search source files with ast-grep patterns.
pub fn find(params: AstFindParams, ct: &CancelToken) -> Result<AstFindResult> {
	let AstFindParams {
		patterns,
		lang,
		path,
...
/// Match ast-grep patterns against an in-memory source string.
///
/// This is the file-free counterpart to [`find`]: callers that already hold the
/// source (streaming buffers, generated code, editor contents) avoid a
/// temp-file round trip. `lang` is required since there is no path to infer it
/// from.
pub fn match_source(params: AstMatchParams, ct: &CancelToken) -> Result<AstMatchResult> {
	let AstMatchParams { source, lang, patterns, selector, strictness, limit, offset, include_meta } =
		params;
	...
	if lang_str.is_empty() {
		return Err(anyhow!("`lang` is required for ast_match"));
	}
...
/// Apply ast-grep rewrite rules to matching files; honors `dry_run`.
pub fn edit(params: AstReplaceParams, ct: &CancelToken) -> Result<AstReplaceResult> {
	let AstReplaceParams {
		rewrites,
		lang,
		path,
		glob,
	...
	for candidate in &candidates {
		...
		let source = match std::fs::read_to_string(&candidate.absolute_path) {
			...
			Err(err) => {
				if fail_on_parse_error {
					return Err(anyhow!("{}: {err}", candidate.display_path));
				}
		...
		'patterns: for (_pattern, rewrite, compiled) in &compiled_rules {
			for matched in ast.root().find_all(compiled.clone()) {
				...
				let after = String::from_utf8(edit.inserted_text.clone()).map_err(|err| {
					anyhow!("{}: replacement text is not valid UTF-8: {err}", candidate.display_path)
				})?;
	...
	if !dry_run {
		for write in &pending_writes {
			ct.heartbeat()?;
			std::fs::write(&write.absolute_path, &write.output)
				.map_err(|err| anyhow!("Failed to write {}: {err}", write.absolute_path.display()))?;
		}
	}
	...
	fn ast_edit_dedupes_identical_matches_across_rules() {
		...
		let result = edit(
			AstReplaceParams {
				...
				path:                Some(tree.root.to_string_lossy().into_owned()),
				glob:                None,
				selector:            None,
